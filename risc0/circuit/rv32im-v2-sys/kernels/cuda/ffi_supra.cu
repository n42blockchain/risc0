...
__global__ void eval_check(Fp* check,
  ...
  if (cycle < domain) {
    ...
    // Optimize memory access pattern with coalesced writes
    uint32_t base_idx = cycle;
    check[base_idx] = ret[0];                  // First quarter
    check[base_idx + domain] = ret[1];         // Second quarter
    check[base_idx + (domain * 2)] = ret[2];   // Third quarter
    check[base_idx + (domain * 3)] = ret[3];   // Fourth quarter
  }
}
